...
const colorette_1 = __nccwpck_require__(338);
/* eslint-disable no-console */
const log = (message) => console.log(`${(0, colorette_1.green)('#')} ${String(message)}`);
exports.log = log;
const logWarn = (message) => console.log(`${(0, colorette_1.yellow)('#')} ${String(message)}`);
exports.logWarn = logWarn;
const logError = (message) => console.log((0, colorette_1.red)(`# ${String(message)}`));
...
function run() {
    return __awaiter(this, void 0, void 0, function* () {
        try {
            const packageFolder = core.getInput('packageFolder');
            const feature = core.getInput('feature');
            ...
            const buildParams = (0, utils_1.buildCmdParams)(feature, featureConfig, publish);
            core.info(`Run build command:  "${buildCmd} ${buildParams.join(' ')}"`);
            (0, process_1.spawnSyncLogged)(buildCmd, buildParams, spawnOptions);
            const extensions = (0, utils_1.getResultExtension)();
            ...
            if (extensions[0]) {
                const resultFileName = yield (0, utils_1.findFirstFileByExtension)(fullPathToBuildFolder, extensions[0]);
                core.info(`Result file - ${resultFileName}`);
                core.setOutput('packageFile', resultFileName);
            }
            if (extensions[1]) {
                const resultFileName = yield (0, utils_1.findFirstFileByExtension)(fullPathToBuildFolder, extensions[1]);
...
Object.defineProperty(exports, "__esModule", ({ value: true }));
exports.reportProcessError = exports.spawnSyncLogged = exports.spawnSyncSafe = void 0;
const child_process_1 = __nccwpck_require__(81);
const log_1 = __nccwpck_require__(817);
const language_helpers_1 = __nccwpck_require__(968);
exports.spawnSyncSafe = ((...args) => {
    const spawnResult = (0, child_process_1.spawnSync)(...args);
    if (spawnResult.status !== 0) {
        throw new Error(`Command "${args
            .filter(language_helpers_1.isString)
            .join(' ')}" failed with exit code ${String(spawnResult.status)}.`);
    }
    return spawnResult;
});
function spawnSyncLogged(command, args, options, label = options.cwd || process.cwd()) {
    (0, log_1.log)(`${label}: ${command} ${args.join(' ')}`);
    return (0, exports.spawnSyncSafe)(command, args, options);
}
exports.spawnSyncLogged = spawnSyncLogged;
function reportProcessError(message) {
    (0, log_1.logError)(message);
    process.exitCode = 1;
}
exports.reportProcessError = reportProcessError;
